...
import { GenerationState, GenerationStatus } from '#@/memoriales/components/generation-status';
...
import { getTemplateById, memorialesRegistry } from '#@/memoriales/manifests/registry';
import { plexMonoMemoriales, quicksandMemoriales } from '#@/memoriales/styles/fonts';
...
export default function MemorialesPage() {
  ...
  const formDisabled = status === 'validating' || status === 'generating' || status === 'success';
  const submitLabel = status === 'validating'
    ? 'Validando…'
    : status === 'generating'
      ? 'Generando…'
      : 'Generar documento';
  ...
  return (
    ...
    >
      <header className={styles.header}>
        <Icon name="gavel" className={styles.headerIcon} size={32} />
        <div>
      ...
      <p className={styles.intro}>
        Genere memoriales a partir de las plantillas del despacho y descargue el documento en formato .docx.
      </p>

      {view === 'selection'
        ? <TemplateSelector templates={TEMPLATES} selectedId={selectedId} onSelect={handleSelect} />
        : null}
            ...
            <section className={styles.formSection}>
              <div className={styles.selectedBar}>
                <div className={styles.selectedInfo}>
                  <Icon name="description" className={styles.selectedIcon} size={22} />
                  <div>
                    <div className={styles.selectedLabel}>Memorial seleccionado</div>
                    <div className={styles.selectedName}>{selectedTemplate.displayName}</div>
                  </div>
                </div>
                <Button variant="text" icon={<Icon name="arrow_back" size={18} />} onClick={handleBackToSelection}>
                  Cambiar memorial
                </Button>
              </div>

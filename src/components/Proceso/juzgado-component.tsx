...
export function JuzgadoComponent({ juzgado }: { juzgado: Juzgado }) {
  const { tipo, url, id, ciudad } = juzgado;
  ...
  return (
    <Link
      ...
      target={'_blank'}
      className={
        indexOfTipo === -1 ? styles.buttonOrigen : styles.buttonEjecucion
      }
      href={url as Route}
    >
      <span className={`material-symbols-outlined ${styles.icon}`}>
        {indexOfTipo === -1 ? 'assured_workload' : 'gavel'}
      </span>

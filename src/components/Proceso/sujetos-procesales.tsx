...
export default function SujetosProcesales( {
  ...
  const matcher = sujetosProcesalesRaw.matchAll( /(\s?)([A-Za-z\s/]+)(:)(\s?)([A-Za-z\s.ÓóÚúÍíÁáÉéÑñ()]+)(\|?)/gim, );
  ...
  const objectify = Array.from( mapperObject.entries() );

  return objectify.map( ( object ) => {
    const [
      key,
      value
    ] = object;

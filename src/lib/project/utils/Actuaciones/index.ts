...
function getLatestByDate(actuaciones: intActuacion[]): intActuacion | null {
  ...
  return actuaciones.reduce((prev, current) => {
    const prevDate = ensureDate(prev.fechaActuacion)?.getTime() || 0;
    const currDate = ensureDate(current.fechaActuacion)?.getTime() || 0;
    ...
    if (currDate === prevDate) {
      const prevReg = ensureDate(prev.fechaRegistro)?.getTime() || 0;
      const currReg = ensureDate(current.fechaRegistro)?.getTime() || 0;
      ...
      if (currReg === prevReg) {
        return current.consActuacion > prev.consActuacion ? current : prev;
      }
    }
...
export default async function fetchActuaciones(
  ...
  try {
    ...
    if (!request.ok) {
      throw new Error(
        `request is not ok: ${request.status}: ${request.statusText}`,
      );
    }

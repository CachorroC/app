...
export class Factura implements intFactura {
  ...
  constructor(
    ...
    for (const matchedKeyValues of firstMatcher) {
      ...
      facturaMap.set(matchedKeyValues[1], matchedKeyValues[2]);
    }
  ...
  static convertRawToFactura(qrString: string): incomingRawFactura {
    ...
    if (!pruebaSiExisteNumFactura) {
      ...
      responseFactura = {
        ...
        hasIva: parseInt(newFactura.ValorIVA) === 0 ? false : true,
        hasOtroImp:
          parseInt(newFactura.ValorOtrosImpuestos) === 0 ? false : true,
        hasIcui: parseInt(newFactura.ValorOtrosImpuestos) === 0 ? false : true,
        hasImpoConsumo:
          parseInt(newFactura.ValorOtrosImpuestos) === 0 ? false : true,
      };
    } else {
      ...
      responseFactura = {
        ...
        hasIva: parseInt(newFactura.ValIva) === 0 ? false : true,
        hasOtroImp: parseInt(newFactura.ValOtroIm) === 0 ? false : true,
        hasIcui: parseInt(newFactura.ValOtroIm) === 0 ? false : true,
        hasImpoConsumo: parseInt(newFactura.ValOtroIm) === 0 ? false : true,

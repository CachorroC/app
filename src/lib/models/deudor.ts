...
function telefonoFixer(telefono: string) {
  ...
  return {
    telFijo: fijoNumber ? String(fijoNumber[0]) : null,
    telCelular: celularNumber ? String(celularNumber[0]) : null,
  };
}

export class ClassDeudor implements Deudor {
  constructor({ deudor: deudorRaw, numero }: CarpetaRaw) {
    this.id = numero;
    this.carpetaNumero = numero;

    const { cedula, direccion, email, nombre, telefono = '' } = deudorRaw;

    const { telFijo, telCelular } = telefonoFixer(String(telefono));

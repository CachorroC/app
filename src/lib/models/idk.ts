export function fechaPresentacionBuilder(
  ...
  if (isNumber) {
    return [new Date(rawFechaPresentacion)];
  }

  const { length: rawFechaPresentacionLength } = rawFechaPresentacion;

  if (rawFechaPresentacionLength <= 12) {
    //* Hay solamente una fecha
    const fechaFixed = fixSingleFecha(rawFechaPresentacion);
  ...
  const [firstFecha, secondFecha, thirdFecha, fourthFecha] =
...
export function fixSingleFecha(rawFecha: string) {
  const [rawDay, rawMonth, rawYear] = rawFecha.trim().split('/');
  ...
  const stringDate = new Date(
    Number(rawYear.padStart(4, '20')),
    Number(rawMonth) - 1,
    Number(rawDay.padStart(2, '0')),
  );

...
export class ClassNotificacion implements Notificacion {
  constructor({ demanda, numero }: CarpetaRaw) {
    ...
    const { fisico, certimail, autoNotificado } = notificacion;

    this.certimail = certimail ? (certimail === 'SI' ? true : false) : false;
    this.fisico = fisico ? (fisico === 'SI' ? true : false) : false;
    this.autoNotificado = autoNotificado ? new Date(autoNotificado) : null;

    const the291 = notificacion['291'];

    if (the291) {
      const { fechaRecibido, resultado, fechaAporta } = the291;
    ...
    if (the292) {
      const { fechaRecibido, resultado, fechaAporta } = the292;

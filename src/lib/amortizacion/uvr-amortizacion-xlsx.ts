/**
 * uvr-amortizacion-xlsx.ts
 * ------------------------------------------------------------------
 * Exporta el plan de amortización calculado por `uvr-amortizacion-core` a un
 * archivo .xlsx con fórmulas vivas y refrendamientos (validaciones), usando
 * `xlsx-js-style` (fork de SheetJS que sí aplica rellenos y fuentes vía los
 * objetos `s` adjuntos a cada celda). Solo importar desde código de servidor:
 * arrastra SheetJS al bundle.
 * ------------------------------------------------------------------
 */

import * as XLSX from 'xlsx-js-style';
import { calcularPlanAmortizacion } from './uvr-amortizacion-core';
import type { EntradaAmortizacion } from './uvr-amortizacion-core';

export type {
  CambioTasa, EntradaAmortizacion, FilaAmortizacion, PlanAmortizacion, Tramo 
} from './uvr-amortizacion-core';
...
// Estilos (honrados por `xlsx-js-style`).
const S_TITULO = {
  font: {
    bold : true,
    sz   : 13,
...
function fijar(
  ws: XLSX.WorkSheet, fila: number, col: number, celda: Celda
): void {
...
export function construirLibroAmortizacion( entrada: EntradaAmortizacion ): XLSX.WorkBook {
  ...
  fijar(
    wp, 6, 1, entrada.cuotaEsperadaUVR != null
      ? num(
  ...
  fijar(
    wp, filaRef, 1, plan.tramos.length && entrada.cuotaEsperadaUVR != null
      ? formulaNum(
          difF, plan.tramos[ 0 ].cuota - entrada.cuotaEsperadaUVR, FMT_UVR, S_BOLD
        )
      : formulaTxt(
  ...
  const okCache = entrada.cuotaEsperadaUVR != null
  ...
  fijar(
    wa, 0, 0, txt(
      'Plan de amortización — cuota constante en UVR (sistema francés)', S_TITULO
    )
  );
  const cabeceras = [
    'N° cuota',
  ...
  plan.filas.forEach( (
    ...
    fijar(
      wa, r, 1, formulaFecha(
        `EDATE('${ HP }'!$B$6,A${ rx }-1)`, fechaAExcelSerial( f.fecha ), S_CALC
      )
    );
    // Saldo inicial
    fijar(
...
export function generarXlsxArchivo(
  entrada: EntradaAmortizacion, ruta: string
): void {
  XLSX.writeFile(
...
export function descargarXlsxEnNavegador(
  entrada: EntradaAmortizacion, nombre = 'amortizacion-uvr.xlsx'
): void {
  XLSX.writeFile(

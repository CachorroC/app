...
( () => {
  ...
  function parseDcDocument( doc ) {
    ...
    const {
      props, preview 
    } = parseDataProps( scriptEl?.getAttribute( 'data-props' ) ?? null, );
  ...
  function parseDcText( src ) {
    ...
    const template = src.slice(
      openMatch.index + openMatch[ 0 ].length, close 
    );
  ...
  function boot(
    ...
    const rootName = rootNameForDocument(
      doc, location 
    );
    ...
    if ( ReactDOM.createRoot ) {
      ReactDOM.createRoot( hostEl )
        .render( h( StandaloneRoot ) );
    } else {
      ReactDOM.render(
        h( StandaloneRoot ), hostEl 
      );
    }
  ...
  function resolve(
    ...
    if ( eq ) {
      ...
      const rv = resolve(
        vals, expr.slice( eq.index + eq.op.length ) 
      );
  ...
  var RAW_UNWRAP = Object.fromEntries( Object.entries( RAW_WRAP )
    .map( ( [
      k,
      v
    ] ) => {
  ...
  function cssToObj( css ) {
    ...
    for ( const decl of css.split( ';' ) ) {
      ...
      const prop = decl.slice(
        0, i 
      )
        .trim();
      o[ prop.startsWith( '--' )
        ? prop
        : kebabToCamel( prop ) ] = decl
        .slice( i + 1 )
        .trim();
    }
  ...
  function collectProps(
    ...
    for ( const {
      ...
      if ( name === 'sc-name' || name === 'data-dc-tpl' ) {
        continue;
      }
      ...
      if ( key.startsWith( 'style-' ) ) {
        pseudoClasses.push( host.pseudoClass(
          key.slice( 6 ), value 
        ) );
  ...
  function walkIf(
    ...
    return (
      ...
      if ( v === void 0 && hintGet && ctx?.__streamingNow ) {
        v = hintGet( vals );
      }
  ...
  function walkComponent(
    ...
    const styleGet = styleRaw != null
      ? compileAttr( styleRaw )
      : null;
    const {
      propGetters, hintSize 
    } = collectProps(
      el, true, host 
    );
    const kids = walkChildren(
    ...
    return (
      ...
      const props = {
        ...
        __hostStyle: styleGet
          ? hostPositionStyle( styleGet( vals ) )
          : void 0,
      };
      ...
      return h(
        host.component( name ), props 
      );
    };
  }

  function walkXImport(
    el, host 
  ) {
    ...
    return (
      ...
      const globalName = evalName(
        globalNameGet, vals 
      );
      const name = globalName || evalName(
        exportNameGet, vals 
      );
      const C
        = !name || urlBindable
          ? null
          : globalName
            ? host.resolveExternalGlobal(
                url, globalName 
              )
            : host.resolveExternal(
                url, name 
              );
      const hostStyle = styleGet
        ? hostPositionStyle( styleGet( vals ) )
        : void 0;
      const wrapper = wrap
        ? {
  ...
  var StreamableLogic = class {
    ...
    setState(
      ...
      this.__host && this.__host.__setLogicState(
        update, cb 
      );
    }
    forceUpdate() {
  ...
  function evalDcLogic( src ) {
    ...
    return fn(
      StreamableLogic, StreamableLogic, getReact() 
    );
  }
  ...
  function Placeholder( {
    name, hintSize, streaming, error 
  } ) {
    const [
      w,
      hgt
    ] = ( hintSize || '100%,60px' ).split( ',' );

    return h(
      'div',
      {
        className: 'sc-placeholder' + ( streaming
          ? ' sc-streaming'
          : '' ),
        style: {
          width : w.trim(),
  ...
  function createComponentFactory(
    ...
    class StreamableComponent extends React.Component {
      constructor( props ) {
        ...
        __publicField(
          this, '__streamingNow', false 
        );
        __publicField(
          this, '__htmlStreamingNow', false 
        );
        ...
        this.__makeLogic(
          registry.get( this.__name ).Logic, null 
        );
      ...
      static getDerivedStateFromError( e ) {
        return {
          __err: e instanceof Error && e.message
            ? e.message
            : String( e ),
        };
      }
      ...
      __userProps() {
        const {
          __name, __hintSize, __tplId, __hostStyle, ...rest 
        }
          = this.props;
      ...
      __reconcileLogic() {
        ...
        if (
          ...
          || ( Next === this.__failedLogic
            ...
            && shallowEqual(
              this.__userProps(), this.__failedUserProps 
            ) )
        ) {
          return;
        }
      ...
      render() {
        ...
        const chain = Array.isArray( this.context )
          ? this.context
          : [];
  ...
  function createExternalModules( onResolved ) {
    ...
    function load(
      ...
      console.info(
        '[dc-runtime] x-import: loading', url, '(' + kind + ')' 
      );
      const ready = kind === 'jsx'
        ? ensureBabel()
        : Promise.resolve();
      ready
        .then( () => {
        ...
        .then( ( src ) => {
          ...
          //! nosemgrep: eval-and-function-constructor
          new Function(
            'React', 'module', 'exports', 'require', code 
          )(
            getReact(),
            module,
            module.exports,
    ...
    function waitForGlobal( name ) {
      ...
      const tick = () => {
        const found = isCE
          ...
          : isRenderableType( resolveDottedPath(
              window, name 
            ) );
    ...
    function resolveGlobal(
      ...
      const g = entry.globals[ name ] ?? resolveDottedPath(
        window, name 
      );
    ...
    return (
      ...
      const sel
        = pseudo === 'before' || pseudo === 'after'
          ...
          : '.' + cls + ':' + pseudo;
      el.sheet.insertRule(
        sel + '{' + css + '}', el.sheet.cssRules.length 
      );
      cache.set(
        k, cls 
      );
  ...
  function createRuntime( doc = document ) {
    ...
    const host = {
      ...
      resolveExternalGlobal: (
        url, name 
      ) => {
        return external.resolveGlobal(
          url, name 
        );
      },
      resolveExternalError: (
        url, name 
      ) => {
        return external.getError(
          url, name 
        );
      },
      pseudoClass,
    ...
    function updateHtml(
      ...
      try {
        r.tpl = compileTemplate(
          html, host 
        );
      } catch ( e ) {
        console.error(
          '[dc-runtime] template compile FAILED for', name, e 
        );
      }
    ...
    return {
      ...
      templateSource: ( name ) => {
        return registry.get( name ).html || null;
      },
      StreamableLogic,
    };
  }
  ...
  function init() {
    ...
    const api = {
      ...
      __dcAnnotatedTemplate: ( name ) => {
        return runtime.annotatedTemplate( name );
      },
      /** Editor bridge — the *original* (decoded) template source. */
      __dcTemplateSource: ( name ) => {
        return runtime.templateSource( name );
      },
      __dcBoot: () => {
        rootName = boot(
          runtime, document 
  ...
  loadReactUmd()
    ...
    .catch( ( err ) => {
      console.error(
        '[dc] failed to load React or boot:', err 
      );

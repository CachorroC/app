  ...
  groups: [
    {
      ...
      fields: [{ name: 'nombre', label: 'Nombre del deudor', type: 'text', required: true }],
    },
    {
      key: 'juzgado',
      legend: 'Juzgado',
      fields: [
        { name: 'tipo', label: 'Tipo de juzgado', type: 'text', required: true }, // revisar: ¿select? p.ej. "Civil Municipal"
        { name: 'numero', label: 'Número del juzgado (dígitos)', type: 'text', required: true },
        { name: 'numero_escrito', label: 'Número del juzgado (en letras)', type: 'text', required: true }, // revisar: derivar de `numero`
        { name: 'ciudad', label: 'Ciudad del juzgado', type: 'text', required: true },
      ],
    },
    {
      key: 'radicado',
      legend: 'Radicado',
      fields: [
        { name: 'numero', label: 'Número de radicado', type: 'text', required: true, format: 'radicado' },
        { name: 'año', label: 'Año del radicado', type: 'number', required: true },
      ],
    },
    {
      legend: 'Datos del proceso',
      fields: [
        { name: 'cuantia_value', label: 'Cuantía del proceso', type: 'currency', required: true, format: 'currencyCOP' },
        { name: 'tipo_proceso', label: 'Tipo de proceso', type: 'text', required: true }, // revisar: ¿select? p.ej. "Ejecutivo"
      ],
    },
  ],
};
...
export const aportando291y292: MemorialTemplate = {
  ...
  groups: [
    ...
    },
    {
      ...
      legend: 'Juzgado',
      fields: [
        { name: 'tipo', label: 'Tipo de juzgado', type: 'text', required: true },
        { name: 'numero', label: 'Número del juzgado (dígitos)', type: 'text', required: true },
        { name: 'numero_escrito', label: 'Número del juzgado (en letras)', type: 'text', required: true },
        { name: 'ciudad', label: 'Ciudad del juzgado', type: 'text', required: true },
      ],
    },
    {
    ...
    },
    {
      ...
      fields: [
        { name: 'cuantia_value', label: 'Cuantía del proceso', type: 'currency', required: true, format: 'currencyCOP' },
        { name: 'tipoproceso', label: 'Tipo de proceso', type: 'text', required: true }, // ⚠ template usa `tipoproceso` (sin guion bajo); unificar con el otro memorial
        { name: 'fecha_auto', label: 'Fecha del auto', type: 'date', required: true, format: 'dateLong' },
      ],
    },
    {
      legend: 'Anexos',
      fields: [
        { name: 'has_anexos', label: '¿Incluye anexos?', type: 'boolean' },
        { name: 'anexos_list', label: 'Anexos', type: 'stringList', helpText: 'Un anexo por línea.' },
      ],
    },
  ],
};
...
export const getTemplateById = (id: string): MemorialTemplate | undefined =>
  (memorialesRegistry as Record<string, MemorialTemplate>)[id];

...
function joinedTemplateText(docxPath: string): string {
  ...
  for (const name of Object.keys(zip)) {
    if (XML_PART.test(name)) {
      const text = strFromU8(zip[name]).replace(/<[^>]+>/g, '');
...
export function extractDocxTags(docxPath: string): ExtractedTemplate {
  ...
  const resolve = (path: string): string => {
    ...
    const loop = [...loopStack].reverse().find((l) => l.item === head);
    return loop ? [`${loop.source}[]`, ...rest].join('.') : path;
  };
  ...
  while ((m = TOKEN.exec(text)) !== null) {
    ...
    if (forM) {
      ...
      arrays.add(source);
      loopStack.push({ item: forM[1], source });
    } else if (/^endfor$/.test(stmt)) {
      loopStack.pop();
    } else if (ifM) {
      const cond = leadingPath(ifM[1]);
      if (cond) booleans.add(resolve(cond));
    }
  }

  for (const a of arrays) paths.delete(a); // arrays are represented via their [] item paths
  for (const b of booleans) paths.delete(b);
  return {
    paths: [...paths].sort(),
    booleans: [...booleans].sort(),
    arrays: [...arrays].sort(),
  };
}
